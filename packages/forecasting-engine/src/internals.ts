...
export function subtractDecimal(left: string, right: string): string {
  ...
  return formatScaled(difference);
}

export function absoluteDecimal(value: string): string {
  const units = decimalUnits(value);
  return formatScaled(units < 0n ? -units : units);
}

export function squareDecimal(value: string): string {
  const units = decimalUnits(value);
  const product = units * units;
  let rounded = product / DECIMAL_SCALE;
  if ((product % DECIMAL_SCALE) * 2n >= DECIMAL_SCALE) rounded += 1n;
  return formatScaled(rounded);
}
...
export function assertUniqueKeys(values: readonly string[], field: string): void {
  if (values.length === 0) throw new TypeError(`${field} must not be empty`);
  const seen = new Set<string>();
  for (const value of values) {
    assertKey(value, `${field} item`);
    if (seen.has(value)) throw new TypeError(`${field} must be unique`);
    seen.add(value);
  }
}

export function assertOrdered(values: readonly string[], field: string): void {
