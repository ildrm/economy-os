...
export function assertIsoInstant(value: string, field: string): void {
  ...
  if (
    ...
    throw new TypeError(`${field} must be a real UTC instant`);
  }
}
...
export function assertNonnegativeIntegerText(value: string, field: string): bigint {
  if (!INTEGER_PATTERN.test(value)) {
    throw new TypeError(`${field} must be a canonical nonnegative integer string`);
  }
  const parsed = BigInt(value);
  if (parsed > UINT64_MAX) throw new TypeError(`${field} must fit in an unsigned 64-bit integer`);
  return parsed;
}

export function canonicalDecimal(value: number, field = "simulation output"): string {
...
export function cloneCanonical<T>(value: T): T {
  const text = canonicalize(value, "value", new WeakSet());
...
export function uniqueBy<T>(
  ...
  for (const item of items) {
    ...
    if (values.has(value)) throw new TypeError(`${field} contains duplicate ${value}`);
    values.add(value);
  }
}
