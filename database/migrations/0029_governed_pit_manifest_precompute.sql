-- Resolve the immutable series fingerprint once with the requested series
-- catalog preflight. Admission evidence always freezes status as active, while
-- the existing per-row operational status gate remains current and fail-closed.
...
DECLARE
  ...
  resolved_series_manifest jsonb;
  ...
  WHERE observation.series_id = requested_series_id
    ...
    AND (
      ...
      OR (
        ...
        AND admission_evidence.evidence_manifest->'series'
          = resolved_series_manifest
